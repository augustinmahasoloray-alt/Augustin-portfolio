        ...
        <Reveal yOffset={20}>
          <div className="flex flex-col items-start mb-12">
            ...
            <h2 className="font-display font-bold text-3xl sm:text-4xl text-neutral-900 dark:text-white tracking-tight">
              {t.certifications.title}<span className="text-accent dark:text-accent-dark">.</span>
            </h2>
        ...
        <StaggerContainer staggerDelay={0.1} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {certificates.map((cert) => (
            <StaggerItem key={cert.id} yOffset={24}>
              <div className="h-full p-6 rounded-2xl bg-white dark:bg-[#121524] border border-neutral-200/80 -neutral-800 shadow-xs hover:shadow-md hover:border-accent/40 dark:hover:border-accent-dark/40 transition-all duration-300 flex flex-col justify-between group">
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <div className="w-10 h-10 rounded-xl bg-accent/10 dark:bg-accent-dark/20 border border-accent/20 -accent-dark/40 flex items-center justify-center text-accent dark:text-accent-dark group-hover:scale-105 transition-transform">
                ...
                <div className="pt-3 border-t border-neutral-100 -neutral-800 flex items-center gap-2 text-xs font-semibold text-emerald-600 dark:text-emerald-400">
                  <CheckCircle2 className="w-4 h-4" />
                  <span>{cert.status === "Validé" ? t.certifications.validated : cert.status}</span>
                </div>
